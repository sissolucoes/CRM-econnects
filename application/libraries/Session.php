...
class Session
{
    ...
    public function append_flashdata($newdata, $newval, $pre = '<p>', $post = "<p/>\n"){

        $old_data = $this->flashdata($newdata);

        $merge_data = $old_data .  $pre . $newval . $post;

        $this->set_flashdata($newdata, $merge_data);
